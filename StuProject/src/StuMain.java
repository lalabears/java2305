...
public class StuMain {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			switch(choice) {
			case 1:
				count = stuInput(count, s,name,kor,eng, math);
				break;
			case 2:
				...
				for (int i = 0; i < count; i++) {
					System.out.printf("%d\t%s\t%d\t%d\t%d\t%d\t%.2f\t%d", s[i].stuNo,s[i].name,s[i].kor,s[i].eng,s[i].math,s[i].total,s[i].avg,s[i].rank);
					System.out.println();
				}
	...
	static int stuInput(int count, Student[] s,String name, int kor, int eng, int math ) {
